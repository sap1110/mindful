 ...
 *
 * The split, the shuffle and the epoch count are all fixed, so the lexical
 * heads reproduce exactly. The embedding heads do not reproduce bit-for-bit —
 * the quantised encoder is not deterministic across runs — so compare the
 * printed metrics rather than the artefact bytes. See `logreg.ts`.
 */
...
for (const [task, classes] of [
  ...
  // attribute for `.json` while Vite and the test runner disagree about it; a
  // generated module imports identically everywhere. This is also the file the
  // app actually reads, so `npm run train` has to write *this* — it wrote
  // `.json` for a while, which meant re-training silently changed nothing.
  for (const [features, fitted] of [
    ['embedding', embedding],
    ['lexical', lexical],
  ] as const) {
    writeFileSync(
      join(OUTPUT, `${task}-${features}.ts`),
      '/** Generated by `npm run train`. Do not edit — re-run the trainer. */\n' +
        `export default ${JSON.stringify(fitted.model)} as const\n`,
    )
  }
}
